...
object MotionProfileGenerator {
    ...
    fun generateFlippedProfile(constraints: MotionProfileConstraints, goal_state: MotionProfileGoal, prev_state: MotionState): MotionProfile {
    ...
    @Synchronized fun generateProfile(constraints: MotionProfileConstraints, goal_state: MotionProfileGoal, prev_state: MotionState): MotionProfile {
        var delta_pos: Double = goal_state.pos() - prev_state.pos()
        if (delta_pos < 0.0 || (delta_pos == 0.0 && prev_state.vel() < 0.0)) {
            return generateFlippedProfile(constraints, goal_state, prev_state)
        }

        var start_state: MotionState = MotionState(prev_state.t(), prev_state.pos(), Math.signum(prev_state.vel()) * Math.min(Math.abs(prev_state.vel()), constraints.max_abs_vel()), Math.signum(prev_state.acc()) * Math.min(Math.abs(prev_state.acc()), constraints.max_abs_acc()))
